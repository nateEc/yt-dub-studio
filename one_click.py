...
class OneClick():
    ...
    def oc_install_webui(cls, app_name: str, is_update = False):
        ...
        cls.oc_run_cmd('python -m pip install "setuptools<81"', assert_success=True, environment=True)
    ...
    def install_conda_packages(cls, app_name, selected_gpu='NVIDIA'):
        ...
        # Install mkl with retry logic. Apple Silicon macOS uses Accelerate/OpenBLAS
        # builds and mkl is not available for osx-arm64 in the configured channels.
        if cls.is_macos() and cls.is_arm64():
            print("Skipping mkl installation on macOS arm64.")
        else:
            max_retries = 3
            retry_count = 0
            success = False
