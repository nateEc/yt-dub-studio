...
def tts_f5_multi_tab(user_config: UserConfig):
    tts = GradioF5TTS(user_config)
    voice_spk1 = GradioCelebVoice(user_config)
    voice_spk2 = GradioCelebVoice(user_config)
    system = platform.system()
    
    with gr.Row():
        with gr.Column(scale=4):                     
            with gr.Group():                            
                gr.HTML(f'<center><h4>{i18n("Speaker 1")}</h4></center>')
                language_radio_spk1 = gr.Radio(choices=voice_spk1.gradio_f5_languages(), label=i18n("Language"), value="English")
                celeb_voice_dropdown_spk1 = gr.Dropdown(label=i18n("Voice"), choices=voice_spk1.gradio_voices(), value=None)                                                    
        ...
        with gr.Column(scale=4):                     
            with gr.Group():                            
                gr.HTML(f'<center><h4>{i18n("Speaker 2")}</h4></center>')
                language_radio_spk2 = gr.Radio(choices=voice_spk2.gradio_f5_languages(), label=i18n("Language"), value="English")
                celeb_voice_dropdown_spk2 = gr.Dropdown(label=i18n("Voice"), choices=voice_spk2.gradio_voices(), value=None)                                                    
    ...
    language_radio_spk1.change(voice_spk1.gradio_change_language,
                        inputs=[language_radio_spk1],
                        outputs=[celeb_voice_dropdown_spk1])      
    
    celeb_voice_dropdown_spk1.change(voice_spk1.gradio_change_voice,
                            inputs=[celeb_voice_dropdown_spk1],
                            outputs=[celeb_audio_spk1, celeb_transcript_spk1, celeb_image_spk1])
    
    celeb_audio_spk1.clear(voice_spk1.gradio_clear_voice,
                      inputs=None,
                      outputs=[celeb_transcript_spk1, celeb_image_spk1])    
    
    add_button_spk1.click(tts.gradio_add_label_spk1,
                          inputs=[dubbing_text_in],
                          outputs=[dubbing_text_in])
        
        
    language_radio_spk2.change(voice_spk2.gradio_change_language,
                        inputs=[language_radio_spk2],
                        outputs=[celeb_voice_dropdown_spk2])      
    
    celeb_voice_dropdown_spk2.change(voice_spk2.gradio_change_voice,
                            inputs=[celeb_voice_dropdown_spk2],
                            outputs=[celeb_audio_spk2, celeb_transcript_spk2, celeb_image_spk2])
    
    celeb_audio_spk2.clear(voice_spk2.gradio_clear_voice,
                      inputs=None,
                      outputs=[celeb_transcript_spk2, celeb_image_spk2])        
    
    add_button_spk2.click(tts.gradio_add_label_spk2,
                          inputs=[dubbing_text_in],
                          outputs=[dubbing_text_in])    
    ...
    dubbing_button.click(tts.gradio_tts_dubbing_multi, 
                inputs=[dubbing_text_in, celeb_audio_spk1, celeb_transcript_spk1, celeb_audio_spk2, celeb_transcript_spk2, 
                        model_choice, f5_tts_speed, audio_format_radio], 
