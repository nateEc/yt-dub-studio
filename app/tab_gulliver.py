...
def gulliver_tab(user_config: UserConfig):
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            full_pipeline_button = gr.Button(value=i18n("Run Full Pipeline"), variant="primary")
        ...
        with gr.Column(scale=8):
            ...
            pipeline_status = gr.Textbox(label=i18n("Pipeline Status"), interactive=False, max_lines=8, lines=5)
            with gr.Row():
        ...
        with gr.Column(scale=3):
            ...
            with gr.Group():
                gr.HTML(f'<center><h4>{i18n("Lip Sync")}</h4></center>')
                lip_sync_enabled = gr.Checkbox(
                    label=i18n("Enable Lip Sync"),
                    value=user_config.get("lipsync_enabled", True),
                )
                lip_sync_engine = gr.Dropdown(
                    label=i18n("Lip Sync Engine"),
                    choices=gulliver.get_lipsync_engines(),
                    value=user_config.get("lipsync_engine", "MuseTalk"),
                )
                lip_sync_bbox_shift = gr.Slider(
                    -30,
                    30,
                    value=user_config.get("lipsync_bbox_shift", 0),
                    step=1,
                    label=i18n("BBox Shift"),
                )
                lip_sync_allow_fallback = gr.Checkbox(
                    label=i18n("Allow audio-only fallback"),
                    value=user_config.get("lipsync_allow_audio_only_fallback", True),
                )
    ...
    clear_button.add([input_video, input_audio, transcription_textbox, dubbing_video, dubbing_audio, translation_textbox, pipeline_status])

    full_pipeline_button.click(gulliver.gradio_youtube_pipeline,
                        inputs=[
                            url_text, youtube_quality_radio, audio_format_radio,
                            asr_engine_radio, whisper_model_dropdown, whisper_language_dropdown, compute_type_dropdown, denoise_level,
                            source_language_dropdown, translate_language_dropdown,
                            ms_voice_dropdown, edge_tts_pitch, edge_tts_rate, edge_tts_volume,
                            lip_sync_enabled, lip_sync_engine, lip_sync_bbox_shift, lip_sync_allow_fallback,
                        ],
                        outputs=[
                            input_video, input_audio, transcription_textbox,
                            dubbing_video, dubbing_audio, translation_textbox,
                            dubbing_files, pipeline_status,
                        ])
