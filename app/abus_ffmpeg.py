...
def ffmpeg_replace_audio(input_path: str, audio_path: str, output_path: str, progress = gr.Progress()):
    ...
    if file_extension == ".mp4":
        command = f'ffmpeg -hide_banner -loglevel error -y -i "{input_path}" -i "{audio_path}" -c:v copy -map 0:v:0 -map 1:a:0 -c:a aac -shortest "{output_path}" -nostdin'
    elif file_extension == ".webm":
        command = f'ffmpeg -hide_banner -loglevel error -y -i "{input_path}" -i "{audio_path}" -c:v copy -map 0:v:0 -map 1:a:0 -c:a libopus -shortest "{output_path}" -nostdin'
    else:
        command = f'ffmpeg -hide_banner -loglevel error -y -i "{input_path}" -i "{audio_path}" -c:v copy -map 0:v:0 -map 1:a:0 -c:a aac -shortest "{output_path}" -nostdin'
    ...
    if result.returncode != 0:
        raise RuntimeError(f"ffmpeg_replace_audio failed: {result.stderr[-4000:]}")
    return output_path
...
def ffmpeg_trim_seconds(input_path: str, output_path: str, seconds: float, start_seconds: float = 0):
    command = f'ffmpeg -y -ss {start_seconds} -i "{input_path}" -t {seconds} -c copy "{output_path}"'
